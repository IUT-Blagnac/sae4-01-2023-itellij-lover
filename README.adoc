...
=== Mise en place d'un singleton pour la connexion a la base de données

Nous avons créé la classe DatabaseSingleton.java afin de se connecter à la base de données. 
...
Utilisation de Fenetre.affichererreur() pour l'affichage dans une fenêtre d'erreur.
...
=== Séparation de la partie BD et la partie vue du code

==== Fenetre (Désormais Window)

Création de la classe DialogMatch afin d'y mettre toutes les méthodes pour récupérer les informations de ou des matchs dans la base de données. Utilisation de cette classe dans la classe Fenetre pour afficher les informations des matchs.
...
Mise en place de pull request et de branch autres que `main` afin de ne pas empiéter sur le travail de l'autre aussi pour ne pas à avoir à pull tout le temps et avec des risques avec la structure du programme.
